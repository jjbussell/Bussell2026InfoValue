...
% Uses NIDAQ recording of synchronization TTL pulses from Bpod and
% Inscopix nVista system to align Bpod behavioral data with its internal
% clock and timestamps with imaging from nVista
...
% these threshold the analog signal from the NIDAQ (and nVista GPIO channel
% if used)
...
%% BPOD BEHAVIOR VARIABLES TO LOAD
...
%% DAQ SYNC
    ...
    if ~isempty(DAQfiles)
        ...
        for ff=1:numel(DAQfiles)
            ...
            % these may occasionally change. Manually check in necessary
            DAQframeVals=DAQdata(:,1); DAQTime = DAQdata(:,4); DAQBpod1=DAQdata(:,2); DAQBpod2=DAQdata(:,3);
    %         DAQthreshold = 0.05; %0.05 % can be used to update here
   ...
   for ff=1:numel(Bpodfiles)
    ...
    if sum(mouse=='JB425')==5 & sum(day == '20220519')==8
        BpodTimersAll=[BpodTimersAll(1:11351); BpodTimersAll(11354:end)];
    end  
    if sum(mouse=='JB434')==5 & sum(day == '20220825')==8
...
%     end
    if sum(mouse=='JB413')==5 & sum(day == '20211103')==8
        BpodTimersAll=[BpodTimersAll(1:54733); BpodTimersAll(54736:end)];
    end
    if sum(mouse=='JB425')==5 & sum(day == '20220616')==8
...
%% CHECK SYNC
    ...
    % ensure steady, small acculumation of mis-match between device clocks,
    % expect up to ~300ms difference after 1 hr of recording
    % much larger indicates problems in sync signals
...
% flag to toggle between raw and smoothed CNMFE output
...
% flag to test standard Z-scoring of entire session neural activity
