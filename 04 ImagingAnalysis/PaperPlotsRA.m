...
%     {'C_odor1InfoFirst','C_odor1RandFirst'};... %2
%     {'C_odor2A','C_odor2B'};... %3
%     {'C_odor2C','C_odor2D'};... %4
...
saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_EBMActivityIDXFullTrial']),'pdf');

%% EBM INDEX ACTIVITY ONE FIGURE
...
saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_EBMIdx_SUMMARY']),'pdf');
...
%     end
    ...
    xlim([-0.5 1.5+PID])
    saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_EBMActivity_' label]),'pdf');
...
for cd = 1:size(a.compNamesFirst)
    ...
    nsubplot(1,1,1,1);
    e=6;
    ypop=a.absActivityTimeDiff{cd};
    ymean=mean(ypop);
    ysem = nanstd(ypop,[],1) ./ sqrt(size(ypop,1));
    t=a.t{e};
    for j=1:1000
%         plot(t,mean(squeeze(a.activityDifferenceEBMShuffle{cd}(:,:,j))),'Color','k','LineWidth',0.2);
        plot(t,mean(squeeze(a.shuffleDiff{cd}(:,:,j))),'Color',a.grey,'LineWidth',0.2);
    end
    h = fill([t, fliplr(t)], [ymean-ysem, fliplr(ymean+ysem)],'r','EdgeColor','none');
    set(h, 'FaceAlpha', 0.1);
    ...
    ylabel('Calcium activity');
    xlabel('Time from Event Onset (s)');
    title([label ' p = ' num2str(a.absActDiffPostPopSig{cd})]);
    plot([0+PID 0+PID],[-1 +1].*10^10,'color','k','yliminclude','off');
    plot([0.2 0.2],[-1 +1].*10^10,'color',a.grey,'yliminclude','off');
    plot([1.2 1.2],[-1 +1].*10^10,'color',a.grey,'yliminclude','off');
    plot([-1 +1].*10^10,[0 0],'color','k','xliminclude','off');
    ...
    xlim([-0.5 1.5+PID])
    saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_absActivity_' label]),'pdf');
...
for cd = 1:size(a.compNamesFirst)
    ...
    set(fig,'PaperOrientation','landscape');
    ax=nsubplot(1,1,1,1);
    h_for_legend(end+1)=histogram(ax,a.actDiffPostEBM{cd},[-0.5:0.1:2],'EdgeColor','k','FaceColor',a.grey);
    hold on;
    h_for_legend(end+1)=histogram(ax,a.actDiffPostEBM{cd}(a.actDiffIdxEBMSig{cd}<0.05),[-0.5:0.1:2],'EdgeColor','w','FaceColor','k');
    leg = legend(h_for_legend,{'All Cells','Significant Coding Cells'},'Orientation','vertical','Location','northeast','Box','off');
    leg.FontSize = 6;
    ylabel('Cells')
    xlabel([label ' Coding Idx'])
    title([label ' ' num2str(sum(a.actDiffIdxEBMSig{cd}<0.05)/a.neuronCt*100) ' % of cells'])
    saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_EBMActivityHistogram_' label]),'pdf');
end
...
for cd = 1:size(a.compNamesFirst)
    ...
    set(fig,'PaperOrientation','landscape');
    ax=nsubplot(1,1,1,1);
    h_for_legend(end+1)=histogram(ax,a.actDiffPostEBM{cd}-a.actDiffPreEBM{cd},-1:0.1:2,'EdgeColor','k','FaceColor',a.grey);
    hold on;
    h_for_legend(end+1)=histogram(ax,a.actDiffPostEBM{cd}(a.actDiffIdxEBMSig{cd}<0.05)-a.actDiffPreEBM{cd}(a.actDiffIdxEBMSig{cd}<0.05),-1:0.1:2,'EdgeColor','w','FaceColor','k');
    leg = legend(h_for_legend,{'All Cells','Significant Coding Cells'},'Orientation','vertical','Location','northeast','Box','off');
    leg.FontSize = 6;
    ylabel('Cells')
    xlabel([label ' Coding Idx, pre- vs post-event'])
    title([label ' ' num2str(sum(a.actDiffIdxEBMSig{cd}<0.05)/a.neuronCt*100) ' % of cells'])
    saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_EBMActivityPrePostHistogram_' label]),'pdf');
end
...
for cd = 1:size(a.compNamesFirst)
    ...
    set(fig,'PaperOrientation','landscape');
    ax=nsubplot(1,1,1,1);
    h_for_legend(end+1)=histogram(ax,a.activityPostDiff{cd},[-2:0.1:2],'EdgeColor','k','FaceColor',a.grey);
    hold on;
    h_for_legend(end+1)=histogram(ax,a.activityPostDiff{cd}(a.pNeuronsPost{cd}<5),[-2:0.1:2],'EdgeColor','w','FaceColor','k');
    leg = legend(h_for_legend,{'All Cells','Significant Coding Cells'},'Orientation','vertical','Location','northeast','Box','off');
    leg.FontSize = 6;
    ylabel('Cells')
    xlabel([label ' Coding Idx'])
    title(label)
    saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_ActivityHistogram_' label]),'pdf');
end

%% TRIAL MEAN ACTIVITY OVERALL ALL CELLS
...
ypop = mean(a.C_events{11},3,'omitnan');
ymean = mean(ypop,1,'omitnan');
ysem = nanstd(ypop,[],1) ./ sqrt(size(ypop,1));
ymean=ymean(cebrastart:360);
ysem=ysem(cebrastart:360);
t=a.t{e}(cebrastart:360);
h = fill([t, fliplr(t)], [ymean-ysem, fliplr(ymean+ysem)], 'k','EdgeColor','none');
set(h, 'FaceAlpha', 0.1);
plot(t,ymean,'color','k','linewidth',width); % only this plot is used for legend!!

ypop = mean(a.C_trialInfo,3,'omitnan');
ymean = mean(ypop,1,'omitnan');
ysem = nanstd(ypop,[],1) ./ sqrt(size(ypop,1));
ymean=ymean(360:389);
ysem=ysem(360:389);
...
h = fill([t, fliplr(t)], [ymean-ysem, fliplr(ymean+ysem)], a.purple,'EdgeColor','none');
set(h, 'FaceAlpha', 0.1);
plot(t,ymean,'color',a.purple,'linewidth',width); % only this plot is used for legend!!

ypop = mean(a.C_trialRand,3,'omitnan');
ymean = mean(ypop,1,'omitnan');
ysem = nanstd(ypop,[],1) ./ sqrt(size(ypop,1));
ymean=ymean(360:389);
ysem=ysem(360:389);
t=a.t{e}(360:389);
h = fill([t, fliplr(t)], [ymean-ysem, fliplr(ymean+ysem)], a.orange,'EdgeColor','none');
set(h, 'FaceAlpha', 0.1);
plot(t,ymean,'color',a.orange,'linewidth',width); % only this plot is used for legend!!

y=a.C_trialRandForcedC;
...
ysem=ysem(389:593);
t=a.t{e}(389:593);
h = fill([t, fliplr(t)], [ymean-ysem, fliplr(ymean+ysem)], a.cornflower,'EdgeColor','none');
set(h, 'FaceAlpha', 0.1);
plot(t,ymean,'color',a.cornflower,'linewidth',width,'linestyle','--'); % only this plot is used for legend!!

y=a.C_trialRandForcedD;
ypop = mean(y,3,'omitnan');
ymean = mean(ypop,1,'omitnan');
ysem = nanstd(ypop,[],1) ./ sqrt(size(ypop,1));
ymean=ymean(389:593);
...
h = fill([t, fliplr(t)], [ymean-ysem, fliplr(ymean+ysem)], a.cornflower,'EdgeColor','none');
set(h, 'FaceAlpha', 0.1);
plot(t,ymean,'color',a.cornflower,'linewidth',width); % only this plot is used for legend!!

y=a.C_trialInfoForcedBig;
ypop = mean(y,3,'omitnan');
ymean = mean(ypop,1,'omitnan');
ysem = nanstd(ypop,[],1) ./ sqrt(size(ypop,1));
ymean=ymean(389:cebrastop);
ysem=ysem(389:cebrastop);
t=a.t{e}(389:cebrastop);
h = fill([t, fliplr(t)], [ymean-ysem, fliplr(ymean+ysem)],'g','EdgeColor','none');
set(h, 'FaceAlpha', 0.1);
plot(t,ymean,'color','g','linewidth',width); % only this plot is used for legend!!

y=a.C_trialInfoForcedSmall;
ypop = mean(y,3,'omitnan');
ymean = mean(ypop,1,'omitnan');
ysem = nanstd(ypop,[],1) ./ sqrt(size(ypop,1));
ymean=ymean(389:cebrastop);
ysem=ysem(389:cebrastop);
t=a.t{e}(389:cebrastop);
h = fill([t, fliplr(t)], [ymean-ysem, fliplr(ymean+ysem)],'m','EdgeColor','none');
set(h, 'FaceAlpha', 0.1);
plot(t,ymean,'color','m','linewidth',width); % only this plot is used for legend!!

y=a.C_trialRandForcedBig;
ypop = mean(y,3,'omitnan');
ymean = mean(ypop,1,'omitnan');
ysem = nanstd(ypop,[],1) ./ sqrt(size(ypop,1));
ymean=ymean(593:cebrastop);
ysem=ysem(593:cebrastop);
t=a.t{e}(593:cebrastop);
h = fill([t, fliplr(t)], [ymean-ysem, fliplr(ymean+ysem)],'b','EdgeColor','none');
set(h, 'FaceAlpha', 0.1);
plot(t,ymean,'color','b','linewidth',width); % only this plot is used for legend!!

y=a.C_trialRandForcedSmall;
ypop = mean(y,3,'omitnan');
ymean = mean(ypop,1,'omitnan');
ysem = nanstd(ypop,[],1) ./ sqrt(size(ypop,1));
ymean=ymean(593:cebrastop);
ysem=ysem(593:cebrastop);
t=a.t{e}(593:cebrastop);
h = fill([t, fliplr(t)], [ymean-ysem, fliplr(ymean+ysem)],'c','EdgeColor','none');
set(h, 'FaceAlpha', 0.1);
plot(t,ymean,'color','c','linewidth',width); % only this plot is used for legend!!
...
saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_TrialMeanActivity_OVERALL']),'pdf');

%% HEATMAPS

% for each comp with difference
...
saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_ConditionalActivity_CenterOdorbyInfoIDX_byInfoDiff_meansub']),'pdf');
...
%% CENTER ODOR BY SIDE HEATMAPS SORT BY ABS DIFF Info/No

% cell_sort_ids=iASort;
cell_sort_ids = INdiffIdx;
% cell_sort_ids=cellsByWeights(:,1);
e=3;
cd = 1;

ctitle = a.titles{1};
clabels = {'No Info Left','Info Left','No Info Right','Info Right'};
cnames = {'C_odor1FirstRandLeft','C_odor1FirstInfoLeft','C_odor1FirstRandRight','C_odor1FirstInfoRight'};
figure();
fig = gcf;
fig.PaperUnits = 'inches';
fig.PaperPosition = [0 0 11 8.5];
...
ax = nsubplot(1,7,1,4);
y1=mean(a.C_odor1FirstRandLeft,3,'omitnan');
y2=mean(a.C_odor1FirstInfoLeft,3,'omitnan');
y=y2-y1;
t=a.t{e};
...
ax = nsubplot(1,7,1,7);
y1=mean(a.C_odor1FirstRandRight,3,'omitnan');
y2=mean(a.C_odor1FirstInfoRight,3,'omitnan');
y=y2-y1;
t=a.t{e};
imagesc(t,1:size(y,1),y(cell_sort_ids(:,1),:),diff_limits);
% colorcet('D1');
...
saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_ConditionalActivity_CenterOdor_byInfoandSide_byINdiff_Info']),'pdf');
...
[infoSort,cell_sort_ids] = sort(mean(squeeze(mean(a.C_odor1FirstInfoLeft(:,40:60,:),2,'omitnan')),2,'omitnan'),'descend');

e=3;
cd = 1;

ctitle = a.titles{1};
clabels = {'Info Left','Info Right','No Info Left','No Info Right',};
cnames = {'C_odor1FirstInfoLeft','C_odor1FirstInfoRight','C_odor1FirstRandLeft','C_odor1FirstRandRight'};
figure();
fig = gcf;
fig.PaperUnits = 'inches';
fig.PaperPosition = [0 0 11 8.5];
...
hold off;   
ha = axes('Position',[0 0 1 1],'Xlim',[0 1],'Ylim',[0  1],'Box','off','Visible','off','Units','normalized', 'clipping' , 'off');
text(0.5, 0.98,[strjoin(mice,' _ '),' Conditional Activity, sort by Info left'],'FontSize',14,'FontWeight','bold','HorizontalAlignment','center');

saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_ConditionalActivity_CenterOdor_byInfoandSide_byInfoL']),'pdf');

%% CONDITIONAL ACTIVITY BY FORCED AND SIDE
...
ctitle = a.titles{1};
clabels = {'Info Forced Left','Info Forced Right','Info Choice Left','Info Choice Right',};
cnames = {'C_odor1FirstInfoForcedLeft','C_odor1FirstInfoForcedRight','C_odor1FirstInfoChoiceLeft','C_odor1FirstInfoChoiceRight'};
figure();
fig = gcf;
fig.PaperUnits = 'inches';
fig.PaperPosition = [0 0 11 8.5];
...
hold off;   
ha = axes('Position',[0 0 1 1],'Xlim',[0 1],'Ylim',[0  1],'Box','off','Visible','off','Units','normalized', 'clipping' , 'off');
text(0.5, 0.98,[strjoin(mice,' _ '),' Conditional Activity, sort by Info Forced left'],'FontSize',14,'FontWeight','bold','HorizontalAlignment','center');

saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_ConditionalActivity_CenterOdor_byForcedandSide_byInfoFL']),'pdf');

%% CENTER EXIT INFO/NO INFO BY DIFF
% 
% % color_limits = [-1.2 1.2];
% % diff_limits = [-0.6 0.6];
% 
...
% e=3;
% 
% [infoSort,infoIdx] = sort(mean(squeeze(mean(a.C_centerExitInfo(:,40:60,:),2,'omitnan')),2,'omitnan'),'descend');
% [randSort,randIdx] = sort(mean(squeeze(mean(a.C_centerExitRand(:,40:60,:),2,'omitnan')),2,'omitnan'),'descend');
% y_info=mean(a.C_centerExitInfo,3,'omitnan');
% y_info=y_info-mean(y_info(:,30:40),2);
% y_rand=mean(a.C_centerExitRand,3,'omitnan');
% y_rand=y_rand-mean(y_rand(:,30:40),2);
% [~,INdiffIdx] = sort(mean(y_info(:,40:60),2)-mean(y_rand(:,40:60),2),'descend');
% % cell_sort_ids=isASort;
% % cell_sort_ids=infoIdx;
...
% hold off;   
% ha = axes('Position',[0 0 1 1],'Xlim',[0 1],'Ylim',[0  1],'Box','off','Visible','off','Units','normalized', 'clipping' , 'off');
% text(0.5, 0.98,[strjoin(mice,' _ '),' Conditional Activity Center Exit, sort by info-noinfo'],'FontSize',14,'FontWeight','bold','HorizontalAlignment','center');
% 
% if RA==1
% colormap(a.ckr);
% else
...
% end
% 
% saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_ConditionalActivity_CenterExitbyInfo_byInfoDiff_meansub']),'pdf');

%% SIDE ENTRY INFO/NO INFO BY DIFF
...
% e=3;
% 
% [infoSort,infoIdx] = sort(mean(squeeze(mean(a.C_sideEntryInfo(:,40:60,:),2,'omitnan')),2,'omitnan'),'descend');
% [randSort,randIdx] = sort(mean(squeeze(mean(a.C_sideEntryRand(:,40:60,:),2,'omitnan')),2,'omitnan'),'descend');
% y_info=mean(a.C_centerExitInfo,3,'omitnan');
% y_info=y_info-mean(y_info(:,30:40),2);
% y_rand=mean(a.C_centerExitRand,3,'omitnan');
% y_rand=y_rand-mean(y_rand(:,30:40),2);
...
% 
% ax = nsubplot(1,3,1,3);
% y_info=mean(a.C_sideEntryInfo,3,'omitnan');
% y_rand=mean(a.C_sideEntryRand,3,'omitnan');
% y=y_info-y_rand;
% t=a.t{e};
% imagesc(t,1:size(y,1),y(cell_sort_ids,:),diff_limits);
% % colorcet('D1');
...
% hold off;   
% ha = axes('Position',[0 0 1 1],'Xlim',[0 1],'Ylim',[0  1],'Box','off','Visible','off','Units','normalized', 'clipping' , 'off');
% text(0.5, 0.98,[strjoin(mice,' _ '),' Conditional Activity SideEntry, sort by info-noinfo'],'FontSize',14,'FontWeight','bold','HorizontalAlignment','center');
% 
% if RA==1
% colormap(a.ckr);
% else
...
% end
% 
% saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_ConditionalActivity_SideEntrybyInfo_byInfoDiff_meansub']),'pdf');
...
% y_A=y_A-mean(y_A(:,30:40),2);
% [~,Asort] = sort(mean(y_A(:,40:60),2),'descend');
% y_B1=mean(a.C_odor2BLeave,3,'omitnan');
% y_B1=y_B1-mean(y_B1(:,30:40),2);
% y_B2=mean(a.C_odor2BStay,3,'omitnan');
% y_B2=y_B2-mean(y_B2(:,30:40),2);
% [~,B1sort] = sort(mean(y_B1(:,40:60),2),'descend');
% [~,BdiffSort] = sort(mean(y_B1(:,40:60),2)-mean(y_B2(:,40:60),2),'descend');
% % cell_sort_ids = BdiffSort;
% cell_sort_ids = B1sort;
% 
% figure();
% fig = gcf;
% fig.PaperUnits = 'inches';
...
% ax = nsubplot(1,4,1,2);
% t=a.t{e};
% imagesc(t,1:size(y_B1,1),y_B1(cell_sort_ids,:),side_limits);
% colorbar()
% plot([0 0],[-1 +1].*10^10,'w','yliminclude','off');
% plot([0.2 0.2],[-1 +1].*10^10,'w','yliminclude','off');
% axis tight;
...
% ax = nsubplot(1,4,1,3); 
% t=a.t{e};
% imagesc(t,1:size(y_B2,1),y_B2(cell_sort_ids,:),side_limits);
% % colorcet('D1');
% colorbar()
% plot([0 0],[-1 +1].*10^10,'w','yliminclude','off');
% plot([0.2 0.2],[-1 +1].*10^10,'w','yliminclude','off');
...
% end
% 
% saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_ConditionalActivity_SideOdorB_byBleaving']),'pdf');
...
saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_ConditionalActivity_CenterSideCSIdxEBM_byABIdx']),'pdf');

%% INFO AND AB HEATMAPS BY INFO ACTIVITY
...
y_info=mean(a.C_odor1FirstInfoForced,3,'omitnan');
y_info=y_info-mean(y_info(:,30:40),2);
[infoSort,infoIdx] = sort(mean(y_info(:,40:60),2),'descend');
cell_sort_ids=infoIdx;

ctitle = a.titles{2};
clabels = {'Info','No Info','A','B'};
cnames = {'C_odor1FirstInfoForced','C_odor1FirstRandForced','C_odor2A','C_odor2B'};
figure();
fig = gcf;
fig.PaperUnits = 'inches';
fig.PaperPosition = [0 0 11 8.5];
...
% ax=nsubplot(1,7,1,1);
% barh(sASort(cell_sort_ids(:,1)),'Facecolor','k');
% ylabel('Cells')
% xlabel('Info - No Info');
% set(ax, 'Ydir', 'reverse')
% ax.FontSize = 8;
...
ax = nsubplot(1,7,1,4);
y1=mean(a.C_odor1FirstInfoForced,3,'omitnan');
y2=mean(a.C_odor1FirstRandForced,3,'omitnan');
y=y1-y2;
t=a.t{e};
imagesc(t,1:size(y,1),y(cell_sort_ids(:,1),:),diff_limits);
% colorcet('D1');
...
hold off;   
ha = axes('Position',[0 0 1 1],'Xlim',[0 1],'Ylim',[0  1],'Box','off','Visible','off','Units','normalized', 'clipping' , 'off');
text(0.5, 0.98,[strjoin(mice,' _ '),' Conditional Activity, sort by Info Activity'],'FontSize',14,'FontWeight','bold','HorizontalAlignment','center');

saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_ConditionalActivity_CenterSideOdor_byInfoActivity']),'pdf');

%% INFO AND AB HEATMAPS BY A
...
% 
% saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_ConditionalActivity_CenterSideOdor_byA']),'pdf');

%% INFO AND AB HEATMAPS BY INFO-NO INFO DIFF
...
% 
% saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_ConditionalActivity_CenterSideOdor_byInfoNoInfoDiff']),'pdf');


%% INFO AND AB HEATMAPS BY A-B DIFF

% e=6;
% cd = 1;
% 
% % [infoSort,infoIdx] = sort(mean(squeeze(mean(a.C_odor1FirstInfoForced(:,40:56,:),2,'omitnan')),2,'omitnan'),'descend');
% cell_sort_ids=isASortAB;
% 
% ctitle = a.titles{2};
% clabels = {'Info','No Info','A','B'};
% cnames = {'C_odor1FirstInfoForced','C_odor1FirstRandForced','C_odor2A','C_odor2B'};
...
% text(0.5, 0.98,[strjoin(mice,' _ '),' Conditional Activity, sort by A-B'],'FontSize',14,'FontWeight','bold','HorizontalAlignment','center');
% 
% saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_ConditionalActivity_CenterSideOdor_byABDiff']),'pdf');
...
hold off;   
ha = axes('Position',[0 0 1 1],'Xlim',[0 1],'Ylim',[0  1],'Box','off','Visible','off','Units','normalized', 'clipping' , 'off');
text(0.5, 0.98,[strjoin(mice,' _ '),' Conditional Activity, sort by B'],'FontSize',14,'FontWeight','bold','HorizontalAlignment','center');

saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_ConditionalActivity_SideOdorABCD_byB']),'pdf');
...
% hold off;   
% ha = axes('Position',[0 0 1 1],'Xlim',[0 1],'Ylim',[0  1],'Box','off','Visible','off','Units','normalized', 'clipping' , 'off');
% text(0.5, 0.98,[strjoin(mice,' _ '),' Conditional Activity, sort by C'],'FontSize',14,'FontWeight','bold','HorizontalAlignment','center');
% 
% saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_ConditionalActivity_SideOdorABCD_byC']),'pdf');
% 
% %% AB CD HEATMAPS by AB
% 
% e=6;
% cd = 1;
% color_limits = [-1.4 1.4];
...
% [~,Asort] = sort(mean(y_A(:,40:60),2),'descend');
% [~,Bsort] = sort(mean(y_B(:,40:60),2),'descend');
% y_AB=mean(a.C_odor2info,3,'omitnan');
% y_AB=y_AB-mean(y_AB(:,30:40),2);
% [~,ABsort] = sort(mean(y_AB(:,40:60),2),'descend');
% % cell_sort_ids=DIdx;
% cell_sort_ids = ABsort;
% % isASortAB
% 
% ctitle = a.titles{2};
% clabels = {'A','B','C','D'};
...
% hold off;   
% ha = axes('Position',[0 0 1 1],'Xlim',[0 1],'Ylim',[0  1],'Box','off','Visible','off','Units','normalized', 'clipping' , 'off');
% text(0.5, 0.98,[strjoin(mice,' _ '),' Conditional Activity, sort by AB'],'FontSize',14,'FontWeight','bold','HorizontalAlignment','center');
% 
% saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_ConditionalActivity_SideOdorABCD_byAB']),'pdf');


%% AB vs CD HEATMAPS by diff
...
% [CSort,CIdx] = sort(mean(squeeze(mean(a.C_odor2C(:,40:80,:),3,'omitnan')),2,'omitnan'),'descend');
% [DSort,DIdx] = sort(mean(squeeze(mean(a.C_odor2D(:,40:80,:),3,'omitnan')),2,'omitnan'),'descend');
y_info=mean(a.C_odor2info,3,'omitnan');
% y_info=y_info-mean(y_info(:,1:20),2);
y_rand=mean(a.C_odor2rand,3,'omitnan');
% y_rand=y_rand-mean(y_rand(:,1:20),2);
[~,ABCDdiffSort] = sort(mean(y_info(:,40:60),2)-mean(y_rand(:,40:60),2),'descend');
% [~,Asort] = sort(mean(y_A(:,40:60),2),'descend');
% [~,Bsort] = sort(mean(y_B(:,40:60),2),'descend');
% y_AB=mean(a.C_odor2info,3,'omitnan');
% y_AB=y_AB-mean(y_AB(:,30:40),2);
% [~,ABsort] = sort(mean(y_AB(:,40:60),2),'descend');
% cell_sort_ids=DIdx;
cell_sort_ids = ABCDdiffSort;
% isASortAB

ctitle = a.titles{2};
clabels = {'AB','CD'};
cnames = {'C_odor2info','C_odor2rand'};
figure();
fig = gcf;
fig.PaperUnits = 'inches';
fig.PaperPosition = [0 0 11 8.5];
...
ax = nsubplot(1,3,1,3);
y1=mean(a.C_odor2info,3,'omitnan');
y2=mean(a.C_odor2rand,3,'omitnan');
y=y1-y2;
t=a.t{e};
imagesc(t,1:size(y,1),y(cell_sort_ids(:,1),:),color_limits);
% colorcet('D1');
...
hold off;   
ha = axes('Position',[0 0 1 1],'Xlim',[0 1],'Ylim',[0  1],'Box','off','Visible','off','Units','normalized', 'clipping' , 'off');
text(0.5, 0.98,[strjoin(mice,' _ '),' Conditional Activity, sort by AB-CD'],'FontSize',14,'FontWeight','bold','HorizontalAlignment','center');

saveas(fig,fullfile(output_dir,[strjoin(mice,'_'),'_ConditionalActivity_SideOdorAB-CD_bydiff']),'pdf');
...
%         end
            if ci == numel(cname)
                ...
                set(gca,'fontsize',8);
                leg = legend(h_for_legend,cn,'Orientation','vertical','Location','northwest','Box','off');
                leg.FontSize = 6;
                title(ctitle);
                xlim([-0.5+PID 1.2+PID]);
%                 xticks([-2:0.2:2]);
                ylim([-0.05 0.1]);
...
colorcet('D1')
    ...
    plot(ax1,[0+PID 0+PID],[-1 +1].*10^10,'k','yliminclude','off');
...
%             yticks([0 0.25 0.5]);
            set(gca,'fontsize',6); 
            plot([0+PID 0+PID],[-1*10^10 .75],'color','k','yliminclude','off');
            plot([-1 +1].*10^10,[0 0],'color',a.grey,'yliminclude','off');
            plot([a.resp_win(1) a.resp_win(1)],[-1 +1].*10^10,'color',a.grey,'yliminclude','off');
            plot([a.resp_win(2) a.resp_win(2)],[-1 +1].*10^10,'color',a.grey,'yliminclude','off');
            if ~isempty(h_for_legend)
...
% y2=a.absActivityTimeDiff{2};
% y2=abs(mean(a.C_odor1InfoForced,3,'omitnan')-mean(a.C_odor1RandForced,3,'omitnan'));
