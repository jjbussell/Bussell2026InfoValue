...
%     days={{'20240517'};{'20240516','20240517'};{'20240515','20240516'}};
    ...
    for m=1:numel(mice)
        ...
        for d=1:numel(days{m})
            ...
            baseDir = ['D:\Bussell Dropbox\Bussell Lab\BpodInfoseek\Analysis\',mouse,'_pipeline'];
            neuroDir = baseDir;
            BpodDir = baseDir;
            SyncDir = baseDir;
            output_dir = baseDir;
    ...
    %             BpodScopeAll = sort([Bpod1Scope;Bpod2Scope]);
        ...
        %% DAQ SYNC
            if ~isempty(DAQfiles)
                ...
                a.DAQFile=[];
                a.DAQFramesFile=[];
                a.DAQCam=[]; a.DAQCamFile=[]; a.DAQCamOff=[];
                a.breaks=[];
                a.DAQTrig=[];
                a.DAQTrigFile=[];
                for ff=1:numel(DAQfiles)
                    ...
                    if size(DAQdata,2)>4
                        DAQframeVals=DAQdata(:,1); % SYNC from Inscopix
                        DAQTime = DAQdata(:,5); % Timestamp from NIDAQ
                        DAQBpod1=DAQdata(:,2); % Bpod Global Timer 7
                        DAQcam = DAQdata(:,4);
                        DAQBpod2=DAQdata(:,3); % Bpod Global Timer 8                    
                    else
                        DAQframeVals=DAQdata(:,1); % SYNC from Inscopix
                        DAQTime = DAQdata(:,4); % Timestamp from NIDAQ
                        DAQBpod1=DAQdata(:,2); % Bpod Global Timer 7
                        DAQBpod2=DAQdata(:,3); % Bpod Global Timer 8                    
                    end
                    ...
                    % sync from camera
    %                 camframesDiff=diff(DAQcam>2.5);
                    camframesDiff=diff(DAQcam>params.DAQthreshold+0.5);
                    camframesLocs=find(camframesDiff==1)+1;
                    camframesOffLocs=find(camframesDiff==-1)+1;
                    camFrames=DAQTime(camframesLocs);
                    camFramesOff=DAQTime(camframesOffLocs);
                    camFramesFile(1:numel(camFrames),1)=ff;

                    a.DAQFrames=[a.DAQFrames; DAQFrames];
                    a.DAQFramesFile=[a.DAQFramesFile;DAQFramesFile];
                    a.DAQCam=[a.DAQCam;camFrames];
                    a.DAQCamOff=[a.DAQCamOff;camFramesOff];
                    a.DAQCamFile=[a.DAQCamFile;camFramesFile];
                    a.DAQSync= [a.DAQSync; BpodDAQAll];
                    a.DAQFile=[a.DAQFile;DAQfile];
                    a.DAQTrig=[a.DAQTrig;BpodCamAll];
                    a.DAQTrigFile=[a.DAQTrigFile;DAQCamfile];
        ...
        %        a.(expandVars{i})=NaN(a.trialCt,maxLength);      
           ...
           for ff=1:numel(Bpodfiles)
               ...
               %% FIND TRIGGERS TOO CLOSE TO TRIAL END (SO NEXT FIRST FRAME IGNORED)

               %Put this above before processing timer8NaN
                trialLengths=a.endTime-a.startTime;
                lastFrames=sum(~isnan(a.GlobalTimer8_Start),2);
                idx=1:size(a.GlobalTimer8_Start,1);
                linear_indices = sub2ind(size(a.GlobalTimer8_Start), idx', lastFrames);
                maxTime=a.GlobalTimer8_Start(linear_indices);
                gaps=trialLengths-maxTime;

                % need threshold for minimum gap!
                threshold=1/(1 / ( 0.010 + ( 1 / 60 ) ));

                tooSoon=gaps<threshold;
                trialsDelFirst=find(tooSoon)+1;
                a.GlobalTimer8_Start(trialsDelFirst,1)=NaN;
                a.GlobalTimer8_End(trialsDelFirst,1)=NaN;

        %         test3=a.DAQCam-a.DAQCam(1);
        %         test4=Bpodtimer8-Bpodtimer8(1);
        %         test5=test3(1:numel(test4));
        % 
        %         % difference in timing between camera output and triggers
        %         test6=test5-test4; %plot this to see jumps, correct number as gaps<threshold?
                % these could be missed frames

                %{
                Delete next trigger (at start of next trial)that comes less than minimum gap-->they will have
                been ignored. Also ignore triggers and, more likely, outputs >
                numel of actual frames. Extra actual frames and outputs in Bpod
                triggered systems from last unsaved trial. In scope-triggered
                systems, recording might have fewer frames due to video problems.
                ...
                if size(timer8timesend,2)==size(timer8times,2)-1
                   timer8timesend=cat(2,timer8timesend(:,:),NaN(size(timer8times,1),1));
                end
        ...
        %         timer8timesend = a.GlobalTimer8_End(a.file==ff,:)+a.startTime(a.file==ff); % set relative to all of Bpod
                ...
                timer7end(timer7endnan~=timer7nan) = imagingEndTimes(timer7trialsend(find(timer7endnan~=timer7nan)));

                timer8nan=isnan(timer8);
                timer8endnan=isnan(timer8end);
                timer8end(timer8endnan~=timer8nan) = imagingEndTimes(timer8trialsend(find(timer8endnan~=timer8nan)));
        ...
        %         BpodTimersAll = sort([timer7all;timer8all]);
                ...
                BpodCamTrig = sort(timer8all);

                BNC1times = a.BNC1High(a.file==ff,:)+a.startTime(a.file==ff); % set relative to all of Bpod
                [BNC1idx,~]=find(BNC1times); % get row (trial) of each time
                BNC1=reshape(BNC1times,[],1); % make single column
                [BNC1trials,BNC1trialidx]=sort(BNC1idx); % sort by trial
                BNC1=BNC1(BNC1trialidx);

                BNC1timesend = a.BNC1Low(a.file==ff,:)+a.startTime(a.file==ff); % set relative to all of Bpod
                [BNC1idxend,~]=find(BNC1timesend); % get row (trial) of each time
                BNC1end=reshape(BNC1timesend,[],1); % make single column
                [BNC1trialsend,BNC1trialidxend]=sort(BNC1idxend); % sort by trial
                BNC1end=BNC1end(BNC1trialidxend);

                cameraEndTimes=a.endTime(a.file==ff);

                BNC1nan=isnan(BNC1);
                BNC1endnan=isnan(BNC1end);
                BNC1end(BNC1endnan~=BNC1nan) = cameraEndTimes(BNC1trialsend(find(BNC1endnan~=BNC1nan)));

                BNC1all = sort([BNC1(~isnan(BNC1)); BNC1end(~isnan(BNC1end))]);

                camSync = BNC1(~isnan(BNC1));
                camSyncend = BNC1end(~isnan(BNC1end));
                camtrials = BNC1trials(~isnan(BNC1));
                camSynctrialsend = BNC1trialsend(~isnan(BNC1end));

                BNC1alltrials = sort([BNC1trials(~isnan(BNC1)); BNC1trialsend(~isnan(BNC1end))]);

                camTimersAll = sort([BNC1all]);      

                camFile(1:numel(camTimersAll),1)=ff;

                a.camFile=[a.camFile;camFile];
                a.camSyncTimers = [a.camSyncTimers;camTimersAll];    
                a.camTimersAll = [a.camTimersAll; camTimersAll];        
...
%                 end                   
                ...
                a.BpodCamTrig = [a.BpodCamTrig;BpodCamTrig];
    ...
    %             title(fname,'Interpreter', 'none')
            ...
            %     close gcf;
                ...
                % CAMERA
                if ~isempty(DAQfiles) & ~isempty(a.DAQCam)
                CamTimes=a.DAQCam-a.DAQCam(1);
                CamDAQDiff = CamTimes - DAQFramesRel(1:numel(CamTimes));
                ...
                set(fig,'PaperOrientation','landscape');
                ax = nsubplot(1,1,1,1);
                plot(CamTimes,CamDAQDiff,'.');
                xlabel('Camera time (s)');
                ylabel('Difference in timestamp between DAQ and Camera Output Signals (s)');
    %             title(fname,'Interpreter', 'none');
                saveas(fig,fullfile(output_dir,[nameroot,'_file_',num2str(ff),'_BpodDAQalignment']),'pdf');
            %     close gcf;
                end
...
%             c.errorTypes = a.errorTypes(a.imagingTrials==1);
        ...
        names=strsplit(fname,'_');
        save(fullfile(output_dir,[names{1},'_',names{2},'_neuralNORM.mat']),'-struct','c','-v7.3');
        save(fullfile(infoseekdir,['BpodInfoseekSessions_',params.dataset{1},'.mat']),'session');
        save(fullfile(output_dir,['Params' nameroot '.mat']),'-struct','params');
