...
% baseDir = 'C:\Users\jbuss\Bussell Dropbox\Jennifer Bussell\BpodInfoseek\Analysis\ForPipeline';
baseDir = 'D:\Dropbox\BpodInfoseek\Analysis\JB426_pipeline';
% baseDir = 'D:\Dropbox\BpodInfoseek\Analysis\JB425_pipeline';
% baseDir = 'D:\Dropbox\BpodInfoseek\Analysis\JB413_pipeline';
% baseDir = 'D:\Dropbox\BpodInfoseek\Analysis\JB424_pipeline';
% baseDir = 'D:\Dropbox\BpodInfoseek\Analysis\JB432_pipeline';
% baseDir = 'D:\Dropbox\BpodInfoseek\Analysis\JB431_pipeline';
% baseDir = 'C:\Users\jbuss\Bussell Dropbox\Jennifer Bussell\BpodInfoseek\Analysis\JB424_pipeline';
neuroDir = baseDir;
BpodDir = baseDir;
SyncDir = baseDir;

% neuroDir = 'C:\Users\jbuss\Desktop\ForPipeline';
% BpodDir = 'C:\Users\jbuss\Desktop\ForPipeline';
% SyncDir = 'C:\Users\jbuss\Desktop\ForPipeline';

%% Select mice and days

% load in struct from file?
% need a mat file with excel data (mouse, session) so can select by trial types, pref,
% infoside, reward, delays, etc

mice = {'JB426'};
% days = {'20220616','20220617','20220624','20220628','20220711','20220713','20220804','20220805'};
<<<<<<< HEAD
% days = {'20220128'};
% mice = {'JB413'};
% days = {'20211019'};
...
=======
days = {'20220128'};
% days = {'20211223','20220315'};
% days = {'20220126','20220127','20220131','20220201'};
% days = {'20220218'};
% days={'20220216','20220217','20220218','20220304','20220307','20220308','20220309'};
% days = {{'20211223','20211228','20220310'}};
% mice = {'JB413'};
% days = {'20211015','20211119','20211123','20211124','20211201','20211202','20211208','20211220','20211223'};
% days = {'20220203','20220207','20220217','20220218'};
% days = {'20220120','20220131','20220203',...
%     '20220207','20220217','20220218'}; %'20220105','20220106','20220119',
% mice = {'JB424'};
...
% days = {'20220428'};%,'20220505','20220429','20220502','20220506','20220520','20220526','20220527','20220613','20220614'
% mice={'JB431'};
% days={'20220628','20220629','20220708','20220517','20220518','20220519','20220524','20220525'};
% mice = {'JB425'};
% days={'20220616'};
% mice={'JB433'};
% days={'20220502'};
% mice={'JB434'};
% days={'20220824','20220825'};
% mice = {'JB425'};
...
params.events = {'trialStart'; 'centerEntry'; 'odor1'; 'centerExit';...
    'sideEntry'; 'odor2'; 'outcome'; 'odor1all'; 'odor1first'; 'baseline';'total'; 'leaving'; 'centerExitFirst'};
params.intervals = [2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,18000,2000,2000];
...
loadVars={'mouse','day','trial','startTime','endTime','StartTrial',...
    ...
    'OutcomeDelivery','TimeoutOutcome','correct','info',...
    'outcome','choice','trialType','trialTypes','infoSide','reward',...
    'infoForced','randForced','infoChoice','randChoice',...
    'infoForcedCorr','infoChoiceCorr','randForcedCorr','randChoiceCorr','forcedCorrTrials',...
    'infoBig','infoSmall','randBig','randSmall','rxn','trialLength',...
    'trialLengthCenterEntry','odorDelay','rewardDelay','errorTypes',...
    'odor2type','odorAtrials','odorBtrials','odorCtrials','odorDtrials',...
    'rewardParams'};

expandVars = {'CenterDelay','CenterOdor','GlobalTimer7_Start','GlobalTimer7_End',...
    'GlobalTimer8_Start','GlobalTimer8_End'};

%% LOAD SESSION TABLE

load(fullfile(findInfoseekData(),['BpodInfoseekSessions_',params.dataset{1},'.mat']));
...
neuroFiles=dir('random');
Bpodfiles=dir('random');
% DAQFiles=dir('random');
% GPIOFiles=dir('random');
for m=1:numel(mice)
   for d=1:numel(days)
      filename=fullfile(neuroDir, [mice{m}, '_*', days{d}, '*cells*.mat']);
      neuroFiles=[neuroFiles; dir(filename)];
%       Bpodfilename = [mice{m} '_*Infoseek*' days{d} '*_pp.mat'];
%       Bpodfiles=[Bpodfiles; dir(fullfile(BpodDir,Bpodfilename))];
%       DAQfilename = [mice{m} '_*' days{d} '*DAQout.csv'];
%       DAQfiles=[DAQfiles; dir(fullfile(SyncDir,DAQfilename))];
%       GPIOfilename = [mice{m} '_*' days{d} '*GPIO.csv'];
%       GPIOfiles=[GPIOfiles; dir(fullfile(SyncDir,GPIOfilename))];
   end
end

% neuroFiles=dir(fullfile(neuroDir,'*cells*.mat'));
% 
% % for these use neuron names and find in appropriate folder
% Bpodfiles=dir([BpodDir,'/*Infoseek*_pp.mat']);
% DAQfiles=dir([SyncDir,'/*DAQout.csv']);
% GPIOfiles=dir([SyncDir,'/*GPIO.csv']);

nfiles=numel(neuroFiles); % base on neural data

%% Output DIRECTORY

% output_dir = 'C:\Users\jbuss\Desktop\ForPipeline';
output_dir = baseDir;

f=1;
%% FOR EACH FILE
for f=1:nfiles
%     currFile=neuroFiles(f).name;
    ...
    Bpodfilename = [mouse '_*Infoseek*' day '*_pp.mat'];
    Bpodfiles=dir(fullfile(BpodDir,Bpodfilename));
    
    if numel(Bpodfiles)~=numel(DAQfiles)
        disp('ERROR: different number of Bpod and DAQ files!!!');
    end
...
%         opts.Delimiter = ",";
%         opts.VariableNames = ["Times", "ChannelName", "Value"];
%         opts.VariableTypes = ["double", "string", "double"];
%         opts.ExtraColumnsRule = "ignore";
%         opts.EmptyLineRule = "read";
%         opts = setvaropts(opts, "ChannelName", "WhitespaceRule", "preserve");
%         opts = setvaropts(opts, "ChannelName", "EmptyFieldRule", "auto");
...
%        a.(expandVars{i})=NaN(a.trialCt,maxLength);      
   ...
   for ff=1:numel(Bpodfiles)
    ...
    if sum(mouse=='JB426')==5 & sum(day == '20220302')==8 % for 426 3/02!!!!!!
        BpodTimersAll=[BpodTimersAll(1:52211); BpodTimersAll(52214:end)];
    end
    if sum(mouse=='JB426')==5 & sum(day == '20220201')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:32301); BpodTimersAll(32304:end)];
    end
    if sum(mouse=='JB426')==5 & sum(day == '20220329')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:8526); BpodTimersAll(8529:end)];
    end
    if sum(mouse=='JB426')==5 & sum(day == '20220309')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:19414); BpodTimersAll(19417:26364); BpodTimersAll(26367:end);];
    end
    if sum(mouse=='JB426')==5 & sum(day == '20220308')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:20278); BpodTimersAll(20281:end)];
    end  
    if sum(mouse=='JB426')==5 & sum(day == '20220304')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:27564); BpodTimersAll(27567:84584); BpodTimersAll(84587:end)];
    end
    if sum(mouse=='JB426')==5 & sum(day == '20220217')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:29758); BpodTimersAll(29761:end)];
    end
    if sum(mouse=='JB426')==5 & sum(day == '20220218')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:35604); BpodTimersAll(35607:end)];
    end
    if sum(mouse=='JB432')==5 & sum(day == '20220527')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:7956); BpodTimersAll(7959:25904); BpodTimersAll(25907:end)];
    end
    if sum(mouse=='JB426')==5 & sum(day == '20220713')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:25965); BpodTimersAll(25968:end)];
    end
    if sum(mouse=='JB426')==5 & sum(day == '20220711')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:37753); BpodTimersAll(37756:end)];
    end    
    if sum(mouse=='JB426')==5 & sum(day == '20220617')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:26775); BpodTimersAll(26778:end)];
    end
    if sum(mouse=='JB426')==5 & sum(day == '20220406')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:54857); BpodTimersAll(54860:end)];
    end    
    if sum(mouse=='JB426')==5 & sum(day == '20220427')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:37140); BpodTimersAll(37143:end)];
    end 
    if sum(mouse=='JB433')==5 & sum(day == '20220621')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:48210); BpodTimersAll(48213:end)];
    end    
    if sum(mouse=='JB433')==5 & sum(day == '20220630')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:34786); BpodTimersAll(34789:end)];
    end
    if sum(mouse=='JB432')==5 & sum(day == '20220623')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:35719); BpodTimersAll(35722:end)];
    end
    if sum(mouse=='JB434')==5 & sum(day == '20220527')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:73196); BpodTimersAll(73199:end)];
    end
    if sum(mouse=='JB413')==5 & sum(day == '20211019')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:22451); BpodTimersAll(22453:22562); BpodTimersAll(22564:end)];
    end
    if sum(mouse=='JB413')==5 & sum(day == '20211019')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:9480); BpodTimersAll(9482:end)];
    end
    if sum(mouse=='JB425')==5 & sum(day == '20220519')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:11351); BpodTimersAll(11354:end)];
    end  
    if sum(mouse=='JB434')==5 & sum(day == '20220825')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:4379); BpodTimersAll(4382:end)];
    end 
%     if sum(mouse=='JB413')==5 & sum(day == '20211019')==8 % for 426 2/1!!!!!!
%         BpodTimersAll=[BpodTimersAll(1:9412); BpodTimersAll(9415:9478);BpodTimersAll(9482:end)];
% %         BpodTimersAll=[BpodTimersAll(1:9412); BpodTimersAll(9415:end)];
%     end
    if sum(mouse=='JB413')==5 & sum(day == '20211103')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:54733); BpodTimersAll(54736:end)];
    end
    if sum(mouse=='JB425')==5 & sum(day == '20220616')==8 % for 426 2/1!!!!!!
        BpodTimersAll=[BpodTimersAll(1:39956); BpodTimersAll(39959:70210); BpodTimersAll(70213:end)];
    end        
...
%% CHECK SYNC
    ff=1;
    for ff=1:numel(Bpodfiles)
        BpodTimers=a.BpodTimersAllRel(a.BpodFile==ff);
    % Bpod vs DAQ
    if ~isempty(DAQfiles)
    ...
    if numel(BpodDAQAllRel)<numel(a.BpodTimersAllRel(a.BpodFile==ff))
...
%% CLEAR LARGE DATA
    clearvars -except a params session loadVars expandVars output_dir SyncDir nfiles neuroFiles baseDir neuroDir nameroot mice fname filepath f DAQfiles GPIOfiles Bpodfiles BpodDir
...
%% SYNC NEURAL DATA
    ...
    % more frames in DAQ than in scope video
    if a.neuronFrames<a.frameCount % neural data for only part of session (< behavior), maybe bad video, only partial recording
       if numel(Bpodfiles)==1
       disp('Fewer neural video frames than behavior session.'); 
       firstFrame = a.frameCount-a.neuronFrames;
       firstFrameTime = a.frameTimes{1}(firstFrame);
       a.frameTimes{1} = a.frameTimes{1}(firstFrame+1:end);
       a.frameCount = size(a.frameTimes,1);
    ...
    if a.frameCount<a.neuronFrames
        disp('More neural data than behavior frames!!!')
        if numel(Bpodfiles)==1
%         if a.neuronFrames == a.frameCount+numel(a.imagingFiles)
%             a.C_smoothed = neuron.C(:,2:end);
%             a.C_raw = neuron.C_raw(:,2:end);       
%         end
        a.C_smoothed = neuron.C(:,1:a.frameCount); %a.neuronFrames-a.frameCount+1:end
...
%% NORMALIZE CELL ACTIVITY
    ...
    if params.normalize == 1
        ...
        for u = 1:a.neuronCt
            ...
            a.C_norm(u,:) = (a.C(u,:) - uactivity_mean(u)) ./ uactivity_std(u);
        end
        a.C = a.C_norm;
    end
...
%     a.imagingTrialsList = trials; % break this down by files!
    ...
    for ff=1:numel(Bpodfiles)
        ...
        trials=a.imagingTrials==1&a.file==ff;
        ...
        for e=1:numel(params.events)
            if e~=8 % all except odor1all
                ...
                for i = 1:sum(~isnan(odor1starts))
                    ...
                    okFrames(eventFrames<=0)=0;
                    okFrames(eventFrames>size(fileFrames,2))=0;
                    odor1AllActivity{tt,i} = a.C(:,eventFrames(okFrames==1));
                end
...
%     c.params = params;
    ...
    % 1 = info forced, 2 = info choice, 3 = rand forced, 4 = rand choice
    % correct only? no choice?
    c.imagingChoice = NaN(sum(a.imagingTrials,1),1);
    c.imagingChoice(ismember(c.imagingOutcome,[11 12 13 14]) )= 1;
    c.imagingChoice(ismember(c.imagingOutcome,[2 3 4 5])) = 2;
    c.imagingChoice(ismember(c.imagingOutcome,[17 18 19 20])) = 3;
    c.imagingChoice(ismember(c.imagingOutcome,[6 7 8 9])) = 4;

%%    
    disp('Saving processed neural data.');
    names=strsplit(fname,'_');
    save(fullfile(output_dir,[names{1},'_',names{2},'_neural.mat']),'-struct','c','-v7.3');
    save(fullfile(findInfoseekData,['BpodInfoseekSessions_',params.dataset{1},'.mat']),'session');
    
    clear a
    clear c
end

save(fullfile(output_dir,'InfoseekNeuroAnalysisParams.mat'),'-struct','params');

%%
% load variables
% align timestamps and save alignment w filename
% trim C
% create PSTHs and trial type arrays, file arrays
% further process PSTHs by trial type?
% SAVE (structure across files? just file + trial + variables?) 
