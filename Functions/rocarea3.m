...
%  nvals = numel(vals); 
%  
%  noise = mean(vals) + 5*randn(ntrials,nvals); 
%  signal = repmat(vals,ntrials,1) + 5*randn(ntrials,nvals); 
%  
%  [area,pval] = rocarea3(noise,signal);
%  
%  figure;
%  subplot(2,1,1); plot(area,'.');
%  subplot(2,1,2); plot(pval,'.');
...
% max_i_per_col = nn - zeros(1,nc);
% max_j_per_col = ns - zeros(1,nc);
for c = 1:nc
    i = 1;
    j = 1;
    pct_noise(:) = 1;
    ...
    switch pval_method
        ...
        otherwise
            ...
            for p = 1:nperms
                ...
                permarea(p,:) = rocarea3(Anew(1:nn,:),Anew((nn+1):end,:));
            end;
            for c = 1:nc
